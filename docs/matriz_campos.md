...
| `Cabecalho` | `codigoConglomerado` | `codigoConglomerado` | `documento` | `str` | `C` + 7 dígitos; tamanho 8 | Deve existir no UNICAD | Excel: obrigatória; XML: obrigatória | Remover espaços; não inventar prefixo ou dígitos | Formato local; existência externa fica `NÃO EXECUTADA` sem UNICAD | 12/2020+ | XSD; crítica `DRO001001` |
| `Cabecalho` | `cnpj` | `cnpj` | `documento` | `str` | 8 dígitos | Oito primeiros dígitos do CNPJ da instituição líder | Excel: obrigatória; XML: obrigatória | Remover `.`, `/`, `-` somente se o valor resultar em 8 dígitos conforme a regra; preservar zeros à esquerda | Não validar CNPJ completo; o documento usa somente a raiz de 8 dígitos | 12/2020+ | XSD; instruções |
